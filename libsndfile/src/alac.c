...
{	ALAC_PRIVATE *plac ;
	...
	{	ALAC_ENCODER *penc = &plac->encoder ;
		...
		//uint8_t kuki_data [plac->kuki_size] ;     // by ZWW
...
{	ALAC_DECODER *pdec = &plac->decoder ;
	//uint8_t		byte_buffer [psf->sf.channels * ALAC_BYTE_BUFFER_SIZE] ;        // by ZWW
	...
	if (packet_size > (uint32_t)psf->sf.channels * ALAC_BYTE_BUFFER_SIZE/*sizeof (byte_buffer)*/)       // added uint32_t by ZWW
	{	
